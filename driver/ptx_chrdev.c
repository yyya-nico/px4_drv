				      ...
				      unsigned int cmd, unsigned long arg)
{
	...
	case PTX_BCAS_DETECT_CARD:
	{
		struct ptx_bcas_detect_card detect;
		struct px4_device *px4 = chrdev->device;
		...
		ret = it930x_bcas_detect_card(it930x, &detected);
		if (ret)
			break;

		detect.detected = detected ? 1 : 0;

		if (copy_to_user((void *)arg, &detect, sizeof(detect)))
			ret = -EFAULT;

		break;
	}

	case PTX_BCAS_RESET_CARD:
	{
		struct px4_device *px4 = chrdev->device;
		struct it930x_bridge *it930x = px4->it930x;
	...
	case PTX_BCAS_SEND_DATA:
	{
		struct ptx_bcas_data data;
		struct px4_device *px4 = chrdev->device;
		struct it930x_bridge *it930x = px4->it930x;
		u8 *kbuf = NULL;
		...
		kbuf = kmalloc(data.len, GFP_KERNEL);
		if (!kbuf) {
			ret = -ENOMEM;
			break;
		}

		if (copy_from_user(kbuf, data.buf, data.len)) {
			ret = -EFAULT;
			kfree(kbuf);
			break;
		}

		ret = it930x_bcas_send_data(it930x, kbuf, data.len);
		kfree(kbuf);
		break;
	}

	case PTX_BCAS_RCV_DATA:
	{
		struct ptx_bcas_data data;
		struct px4_device *px4 = chrdev->device;
		struct it930x_bridge *it930x = px4->it930x;
		...
		ret = it930x_bcas_get_data(it930x, kbuf, &data.len);
		if (!ret && copy_to_user(data.buf, kbuf, data.len)) {
			ret = -EFAULT;
		}

		if (!ret && copy_to_user((void *)arg, &data, sizeof(data)))
			ret = -EFAULT;
	...
	case PTX_BCAS_SET_BAUDRATE:
	{
		struct ptx_bcas_set_baudrate baudrate;
		struct px4_device *px4 = chrdev->device;
		struct it930x_bridge *it930x = px4->it930x;
		...
		if (copy_from_user(&baudrate, (void *)arg, sizeof(baudrate))) {
			ret = -EFAULT;
			break;
		}

		switch (baudrate.baudrate) {
		case PTX_BCAS_BAUDRATE_9600:
			it930x_baudrate = IT930X_UART_BAUDRATE_9600;
			break;
		case PTX_BCAS_BAUDRATE_19200:
			it930x_baudrate = IT930X_UART_BAUDRATE_19200;
			break;
		case PTX_BCAS_BAUDRATE_38400:
			it930x_baudrate = IT930X_UART_BAUDRATE_38400;
			break;
		case PTX_BCAS_BAUDRATE_57600:
			it930x_baudrate = IT930X_UART_BAUDRATE_57600;
			break;
		case PTX_BCAS_BAUDRATE_115200:
			it930x_baudrate = IT930X_UART_BAUDRATE_115200;
			break;
		default:
			ret = -EINVAL;
			break;
